...
import { extractTextWithOCR, isFileProcessable, isFileTooLarge } from '../services/ocrService';
...
const PdfUploader = () => {
  ...
  const handleFiles = (files) => {
    ...
    if (newFiles.length > 0) {
      // Verificar tamaño de archivos (API gratuita tiene límite de 1MB)
      const oversizedFiles = newFiles.filter(file => isFileTooLarge(file));
      if (oversizedFiles.length > 0) {
        ...
        // Advertir sobre archivos grandes, pero aún así agregarlos (se intentará comprimir)
        setError(`Advertencia: Los siguientes archivos exceden el límite de 1MB de la API gratuita: ${warnings.join(', ')}. Se intentará comprimir automáticamente.`);
      }
  ...
  const processFile = async (file) => {
    try {
      ...
      const extractedTextData = await extractTextWithOCR(file, progressCallback);
      
      // Guardar el texto extraído en state para la vista previa
      setExtractedText({
        raw: extractedTextData.raw,
        ocr: extractedTextData.ocr,
        bankType: extractedTextData.bankType
      });
      
      // Sanitizar datos personales sensibles
      addLog('Sanitizando información personal sensible...', 'info');
      const sanitizedText = sanitizePersonalData(extractedTextData.ocr);
      addLog('Datos personales sanitizados correctamente', 'success');
      ...
      addUploadedFile({
        ...
        bankType: extractedTextData.bankType || 'Desconocido'
      });
      ...
      setError(`Error al procesar ${file.name}: ${error.message}`);
      ...
      {selectedFiles.length === 0 && !loading && !processLogs.length && (
        <>
          ...
          <Alert severity="warning" sx={{ mt: 1 }}>
            La API gratuita de OCR.space tiene un límite de 1MB por archivo. Los archivos más grandes se intentarán comprimir automáticamente.
          </Alert>
        </>
      )}
    </Paper>
